...
#            ab_cluster = np.sum(current_data[pindex],1) #for SIR-SIR
            ...
            ab_cluster = current_data[pindex,:,0].astype('float')
            #ab_cluster /= n
    ...
    hist = hist / widths[0]
    #for pindex in range(data_1.p_size):
        #print((hist[pindex]*widths[0]).sum())
    ...
    exponent = int ( np.floor(  np.log10( im1.max() ) )  )  #scientific exponent
...
#aspect = 0.0085 / binNum #non coop primary

#aspect = 0.009 #non primary, new set
#plt.subplot(111)
#cs = ax.imshow(im1, interpolation='none', origin = 'lower', aspect = 50/binNum , cmap=plt.cm.gnuplot)

if node_number_normalized:
    aspect *= n
    cs = ax.imshow(im1, interpolation='none', origin = 'lower', aspect = aspect , cmap=plt.cm.gnuplot , extent =[prange[0],prange[-1],  (1 + omitted_bins * binLen)/n ,  1 ] ) #normalized
...
cbar.ax.text(im1.max() * 5, im1.max() / 2, '$\Pi( \\rho_{ab} )$', rotation = 90) # far mode
#cbar.ax.text(im1.max() * 3.5, im1.max() / 2, '$\Pi( \\rho_{ab} )$', rotation = 90) # near mode
