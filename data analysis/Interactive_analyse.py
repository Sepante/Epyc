...
for qindex in range(data_1.q_size):
    ...
    for pindex in range(data_1.p_size):        
        ...
        if(ab_cluster_display):
            ...
            #ab_cluster = current_data[pindex,:,0]+current_data[pindex,:,1]
            
            
            #ab_cluster = current_data[pindex,:,2]
            #ab_cluster = current_data[pindex][1]
    ...
    name_string = data_1.dis_type + ", " + data_1.data_type + ", $n=$" + str(data_1.nrange[data_1.nindex]) +  ", $q=$" + str(data_1.qrange[qindex]) + ", $r=$" + str(data_1.rrange[data_1.rindex])
    #name_string = '$q=p$' + ", $n=$" + str(nrange[nindex]) 
    plt.suptitle(name_string)
    
    name_string = name_string.replace('$','')
    plt.gcf().subplots_adjust(bottom=0.2)
    #plt.savefig(data_1.location + name_string + str(time.gmtime()[0:5]) + ".png", dpi = 1000) # change the resolution of the saved image
    location = data_1.location
    #plt.savefig(+" histogram, "+ "ab, " +name_string+".png")
    plt.show()
